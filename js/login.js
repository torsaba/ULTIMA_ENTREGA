...
document.addEventListener("DOMContentLoaded", () => {
    ...
    inicioSesion.addEventListener("click", function(event) {
        ...
        if (email.includes("@")  && password) { // evalua que sea correcto (condicion que siempre se cumple)
            ...
            mensajeError.textContent = "Usuario o contraseña incorrecta. Intente otra vez.";
            document.getElementById("formulario").appendChild(mensajeError); 
            // esto es innecesario porque siempre da correcto el inicio

            setTimeout(function() { // para que el mensaje de error desaparezca
                mensajeError.remove();
            }, 2000); 
        }
    });
